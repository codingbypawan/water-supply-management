...
export default function Distribution() {
  ...
  const load = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/distributions', { params: { page, limit: 20 } });
      const body = res.data;
      setDistributions(Array.isArray(body.data) ? body.data : []);
  ...
  const handleAdd = async (formData) => {
    try {
      ...
      toast.error(err.response?.data?.error?.message || 'Failed to record distribution');
    }
  };
  ...
  const columns = [
    ...
    { key: 'total_amount', label: 'Total', render: (val) => `₹${Number(val).toLocaleString()}` },
    {
      key: 'payment_status',
      label: 'Status',
      render: (val) => (
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
          ...
          : val === 'partial' ? 'bg-yellow-50 text-yellow-700'
  ...
  const renderMobileCard = (row) => (
    <div key={row.id} className="bg-white rounded-xl border border-gray-200 p-4 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
          row.payment_status === 'paid' ? 'bg-green-50 text-green-700'
          : row.payment_status === 'partial' ? 'bg-yellow-50 text-yellow-700'
          : 'bg-red-50 text-red-600'
        }`}>
          {row.payment_status}
        </span>
      ...
      <div className="flex items-center justify-between text-sm text-gray-500">
        <span>{new Date(row.distribution_date).toLocaleDateString('en-IN')}</span>
        <span>{row.quantity} × ₹{row.rate}</span>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400 text-sm">No distributions recorded</div>
        ) : (
          <>
            {distributions.map(renderMobileCard)}
...
/* ═══════════════════════════════════════════════════════════ */
/*  Quick Distribution Form — search customer, enter qty, go  */
/* ═══════════════════════════════════════════════════════════ */
function QuickDistributionForm({ onClose, onSave }) {
  ...
  const selectCustomer = (c) => {
    ...
    setResults([]);
    setQuantity(String(c.default_container_count || 1));
    // Focus quantity
    setTimeout(() => document.getElementById('dist-qty')?.focus(), 100);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedCustomer) return toast.error('Select a customer');
    if (!quantity || Number(quantity) <= 0) return toast.error('Enter quantity');
    setSaving(true);
    await onSave({
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40">
      <div className="bg-white w-full sm:rounded-2xl sm:max-w-md sm:mx-4 rounded-t-2xl shadow-xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <h3 className="text-lg font-bold text-gray-900">Record Distribution</h3>
          ...
          <div className="relative">
            ...
            <div className="relative">
              ...
              <input
                ...
                autoComplete="off"
                className="w-full pl-10 pr-10 py-3 border border-gray-200 rounded-xl text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
              />
              {selectedCustomer && (
                <button type="button" onClick={() => { setQuery(''); setSelectedCustomer(null); setResults([]); inputRef.current?.focus(); }}
          ...
          {selectedCustomer && (
            <div className="bg-blue-50 rounded-xl p-3 flex items-center gap-3">
              ...
              <div className="text-right flex-shrink-0">
                {selectedCustomer.custom_rate && (
                  <p className="text-xs font-medium text-green-600">₹{selectedCustomer.custom_rate}/unit</p>
                )}
                <p className="text-xs text-gray-400">
                  Due: ₹{Number(selectedCustomer.outstanding_balance || 0).toLocaleString()}
                </p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button type="button"
                ...
                className="w-12 h-12 rounded-xl border border-gray-200 flex items-center justify-center text-xl font-bold text-gray-600 hover:bg-gray-50 active:bg-gray-100 transition-colors flex-shrink-0">
                −
              </button>
              <input
                id="dist-qty"
                ...
                className="flex-1 text-center text-2xl font-bold py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                placeholder="0"
              />
              <button type="button"
        ...
        <div className="p-4 border-t border-gray-100 safe-area-bottom">
          <button
            ...
            disabled={saving || !selectedCustomer || !quantity}
            className="w-full py-3.5 bg-blue-600 text-white rounded-xl text-base font-semibold hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors active:scale-[0.98]"
          >
            {saving ? 'Recording...' : `Record ${quantity ? quantity + ' container' + (Number(quantity) !== 1 ? 's' : '') : 'Distribution'}`}
          </button>
        </div>
      </div>
    </div>
